...
export default function ProfilePage() {
  ...
  if (isLoading || !profile) {
    return <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-sponge-500" /></div>;
  }
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-white mb-6 font-[var(--font-display)] flex items-center gap-2">
        <span className="emoji-icon">👤</span> Profile
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card>
          <CardContent className="flex flex-col items-center py-8">
            ...
            <h3 className="mt-4 text-lg font-bold text-ocean-800 font-[var(--font-display)]">{profile.fullName}</h3>
            <p className="text-sm text-ocean-500">{profile.email}</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex justify-end">
                <Button type="submit" loading={updateProfile.isPending} variant="sponge">
                  💾 Save Changes
                </Button>
              </div>
            </form>
